...
function sliceTokenPositions(positions: Int32Array, tokenStart: number, tokenCount: number, sourceTokenCount: number): Int32Array {
  ...
  throw new Error(`Reference token batch expects ${sourceTokenCount} or ${sourceTokenCount * 4} positions, got ${positions.length}`);
}
...
function tokenPositionAt(positions: Int32Array, token: number, tokenCount: number): number {
  ...
  throw new Error(`Reference token batch expects ${tokenCount} or ${tokenCount * 4} positions, got ${positions.length}`);
}
